...
function Header({ isLoggedIn, onLoginClick, onLogout, userClassification}) {
    ...
    <header className="navbar navbar-expand-lg navbar-light bg-white fixed-top shadow-sm">
      <div className="container">
        ...
        <Link to="/" className="navbar-brand d-flex align-items-center">
          <span className="ms-2 fw-bold text-dark">NTS</span>
        </Link>
        ...
        <div className={`collapse navbar-collapse ${menuOpen ? 'show' : ''}`}>
          <ul className="navbar-nav ms-auto mb-2 mb-lg-0">
            ...
            {/* 로그인한 경우만 표시될 메뉴 */}
            {isLoggedIn && (userClassification === "TEACHER" || userClassification === "ADMIN") && (
              <>
                <li className="nav-item">
            ...
            {isLoggedIn && userClassification === "ADMIN" && (
              <>
          
                <li className="nav-item">
                  <Link to="/admin" className="nav-link text-dark">관리자페이지</Link>
                </li>
              </>
            )}
          </ul>
          ...
          <div className="d-flex ms-lg-3">
            {isLoggedIn ? (
              ...
              <button className="btn btn-dark" onClick={onLoginClick}>로그인</button>
            )}
          </div>
        </div>
      </div>
